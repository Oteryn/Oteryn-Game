...
- Preserve published task history by default. Never create no-op, retrigger or merge-up commits solely because `main` advanced. Use bounded retries and record `WAITING` or `STALLED` when progress cannot continue; keep a candidate frozen during final qualification where appropriate. Merge Queue integrates an unchanged reviewed head with current `main`; only a material repair requires fresh review.
