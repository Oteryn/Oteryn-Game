...
- Prefer repository-native GitHub APIs and repository CI for remote evidence and execution. Use an isolated checkout or worktree for local-Git tracked-file mutation. The only no-worktree tracked-file publication exception is a separately authorized atomic expected-head API **new-candidate** route under the bound META contract. Remote Desktop is denied unless the owner explicitly authorizes the exact invocation.
