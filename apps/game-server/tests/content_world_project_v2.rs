...
fn identity(key: &str) -> ProjectV2Identity {
    ProjectV2Identity { key: format!("oteryn:content.{key}"), revision: "definition-r1".into() }
}

fn reference(family: ProjectV2Family, key: &str) -> ProjectV2DefinitionRef {
    ProjectV2DefinitionRef { family, key: key.into(), revision: "definition-r1".into() }
}

fn candidate() -> ProjectV2Draft {
    let npc = reference(ProjectV2Family::Npc, "oteryn:content.npc.courier");
    let presentation = reference(ProjectV2Family::Presentation, "oteryn:reference.presentation.courier");
    let asset = ProjectV2AssetRef { key: "oteryn:asset.courier".into(), revision: "asset-r1".into() };
    ProjectV2Draft {
        core: core(),
        state: ProjectV2State {
            ...
            declarations: vec![
                ProjectV2Declaration::Npc {
                    ...
                    behavior: None,
                    dialogue: Some(reference(ProjectV2Family::Dialogue, "oteryn:content.dialogue.courier")),
                    services: vec![reference(ProjectV2Family::Service, "oteryn:content.service.courier")],
                },
                ProjectV2Declaration::WorldObject { identity: identity("object.sign"), presentation: Some(presentation.clone()) },
                ProjectV2Declaration::Dialogue { identity: identity("dialogue.courier") },
                ProjectV2Declaration::Service { identity: identity("service.courier") },
                ProjectV2Declaration::Interaction { identity: identity("interaction.courier") },
                ProjectV2Declaration::Quest { identity: identity("quest.courier") },
                ProjectV2Declaration::Transition { identity: identity("transition.courier") },
                ProjectV2Declaration::House { identity: identity("house.courier") },
                ProjectV2Declaration::Encounter { identity: identity("encounter.courier") },
            ],
            ...
            appearance_bindings: vec![ProjectV2AppearanceBinding {
                presentation: presentation.clone(), asset: asset.clone(),
                disposition: ProjectV2Disposition::CandidateOnly,
            }],
            ...
            sources: vec![ProjectV2Source {
                key: "oteryn:source.reference".into(), revision: "source-r1".into(),
                sha256: "97fbfe027f93834bfaef365e4271dbb56b479ba29528e3f00a1b046aae0a7491".into(),
                evidence: ProjectV2EvidenceClass::Unknown,
            }],
            editor: vec![ProjectV2EditorEntry {
                target: npc, display_name: "Courier".into(), description: "Editor only".into(),
                categories: vec!["npc".into()], notes: vec!["Candidate".into()],
...
fn v1_migrates_explicitly_and_retains_its_original_six_document_wire_format() {
    ...
    let parsed = ProjectSnapshot::new(original.documents().clone(), limits())
        .expect("admit v1").parse(limits()).expect("parse v1");
    assert!(parsed.v2().is_none());
    assert_eq!(parsed.canonical_documents(limits()).expect("v1 rewrite").documents(), original.documents());

    let migrated = CanonicalProjectDocuments::from_v2_draft(parsed.migrate_to_v2(), limits()).expect("explicit migration");
    assert_eq!(migrated.documents().len(), 11);
    let v2 = ProjectSnapshot::new(migrated.documents().clone(), limits())
        .expect("admit v2").parse(limits()).expect("parse v2");
    assert_eq!(v2.v2(), Some(&ProjectV2State::default()));
    assert_eq!(v2.canonical_documents(limits()).expect("v2 rewrite").documents(), migrated.documents());
    assert_eq!(v2.lower_reference_source().expect("reference projection").definitions.len(), 1);
}
...
fn all_v2_declarative_families_round_trip_without_lowering_candidates() {
    ...
    assert_eq!(project.canonical_documents(limits()).expect("rewrite").documents(), documents.documents());
    let reference = project.lower_reference_source().expect("reference projection");
    assert_eq!(reference.definitions.len(), 1);
    assert!(reference.placements.is_empty());
    assert!(reference.transitions.is_empty());
    project.link().expect("server-authoritative Reference linker still works");
}
...
fn typed_references_and_author_aliases_fail_closed() {
    ...
    duplicate_alias.state.editor[0].aliases.push("Courier".into());
...
fn v2_inventory_digest_and_bounded_document_count_are_enforced() {
    ...
    changed.get_mut("assets/catalog.json").expect("asset document").push(b' ');
    assert!(ProjectSnapshot::new(changed, limits()).expect("admit").parse(limits()).is_err());
