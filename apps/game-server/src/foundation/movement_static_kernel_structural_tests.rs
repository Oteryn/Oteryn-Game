...
fn prepare_step(
    ...
    let x = source.x.checked_add(dx).ok_or(MoveError::CoordinateOverflow)?;
    let y = source.y.checked_add(dy).ok_or(MoveError::CoordinateOverflow)?;
    let destination = LocalPosition {
        x,
        y,
        floor: source.floor,
    };
    let cell = StaticCellPosition::checked_from_wide(i64::from(x), i64::from(y), i32::from(source.floor))
        .map_err(MoveError::Cell)?;
    match probe.lookup(requested_scope, cell).map_err(MoveError::Cell)? {
...
fn step_once(
    ...
    let proposal = prepare_step(
        carrier, owner, actor, expected, requested_scope, binding, probe, step,
    )?;
...
impl Harness {
    fn new(position: LocalPosition) -> Self {
        ...
        let actor = carrier.admit(&owner, ActorState(1)).expect("admit fixture actor");
...
fn each_cardinal_step_uses_one_exact_candidate_and_commits_on_the_owner() {
    for (direction, x, y) in [
        ...
        let mut fixture = Harness::new(LocalPosition { x: 4, y: 5, floor: 7 });
        let destination = LocalPosition { x, y, floor: 7 };
        let index = fixture.index(destination, StaticCellFact::Qualified(CollisionClass::Walkable));
        let mut probe = CellProbe { index: &index, lookups: 0 };
        let binding = FixtureBinding { context: fixture.snapshot.version.context, static_scope: &fixture.scope };
        let committed = step_once(
            &mut fixture.carrier, &fixture.owner, fixture.actor, fixture.snapshot,
            &fixture.scope, &binding, &mut probe, direction,
        )
        .expect("walkable cardinal step");
        assert_eq!(probe.lookups, 1);
        assert_eq!(committed.version.position, destination);
        assert_eq!(committed.version.revision, fixture.snapshot.version.revision + 1);
        assert_eq!(fixture.carrier.read_position(&fixture.owner, fixture.actor), Ok(committed));
    }
}
...
fn blocked_absent_unqualified_and_conflict_reject_without_position_or_revision_change() {
    for (fact, expected_error) in [
        (Some(StaticCellFact::Qualified(CollisionClass::Blocked)), MoveError::Blocked),
        (None, MoveError::Cell(StaticCellError::Absent)),
        (Some(StaticCellFact::Unqualified), MoveError::Cell(StaticCellError::Unqualified)),
        (Some(StaticCellFact::Conflict), MoveError::Cell(StaticCellError::Conflict)),
    ] {
        let mut fixture = Harness::new(LocalPosition { x: 4, y: 5, floor: 7 });
        let destination = LocalPosition { x: 5, y: 5, floor: 7 };
        let records = fact.map_or_else(Vec::new, |fact| {
            vec![StaticCellFixtureRecord::fixture(
                &fixture.scope,
                StaticCellPosition::fixture(destination.x, destination.y, destination.floor), fact,
            )]
        });
        ...
        let binding = FixtureBinding { context: fixture.snapshot.version.context, static_scope: &fixture.scope };
        assert_eq!(
            step_once(&mut fixture.carrier, &fixture.owner, fixture.actor, fixture.snapshot,
                &fixture.scope, &binding, &mut probe, Step::East),
            Err(expected_error)
        );
        assert_eq!(probe.lookups, 1);
        assert_eq!(fixture.carrier.read_position(&fixture.owner, fixture.actor), Ok(fixture.snapshot));
    }
}
...
fn stale_snapshot_wrong_scope_context_and_coordinate_overflow_reject_before_lookup() {
    let mut fixture = Harness::new(LocalPosition { x: 4, y: 5, floor: 7 });
    let index = fixture.index(LocalPosition { x: 5, y: 5, floor: 7 }, StaticCellFact::Qualified(CollisionClass::Walkable));
    let mut probe = CellProbe { index: &index, lookups: 0 };
    let mut stale = fixture.snapshot;
    stale.version.revision += 1;
    let binding = FixtureBinding { context: fixture.snapshot.version.context, static_scope: &fixture.scope };
    assert_eq!(
        prepare_step(&fixture.carrier, &fixture.owner, fixture.actor, stale,
            &fixture.scope, &binding, &mut probe, Step::East),
    ...
    let wrong_scope = StaticCellScope::fixture(
        fixture.owner.world_id,
        CoordinateFrameRef::new("other-frame").expect("frame"),
        ProductionAtom::new("fixture generation", "movement-fixture-generation").expect("generation"),
        MapRevisionRef::new("movement-fixture-map").expect("map"),
    );
    assert_eq!(
        prepare_step(&fixture.carrier, &fixture.owner, fixture.actor, fixture.snapshot,
            &wrong_scope, &binding, &mut probe, Step::East),
        Err(MoveError::CellScopeMismatch)
    );
    let wrong_binding = FixtureBinding { context: PreProductionPositionContext {
        coordinate_frame_marker: 99, ..binding.context
    }, static_scope: &fixture.scope };
    assert_eq!(
        prepare_step(&fixture.carrier, &fixture.owner, fixture.actor, fixture.snapshot,
            &fixture.scope, &wrong_binding, &mut probe, Step::East),
    ...
    assert_eq!(
        prepare_step(&fixture.carrier, &fixture.owner, wrong_actor, fixture.snapshot,
            &fixture.scope, &binding, &mut probe, Step::East),
    ...
    for (position, direction) in [
        (LocalPosition { x: i32::MAX, y: 5, floor: 7 }, Step::East),
        (LocalPosition { x: i32::MIN, y: 5, floor: 7 }, Step::West),
        (LocalPosition { x: 4, y: i32::MAX, floor: 7 }, Step::South),
        (LocalPosition { x: 4, y: i32::MIN, floor: 7 }, Step::North),
    ] {
        let mut edge = Harness::new(position);
        let index = ReferenceStaticCellIndex::from_fixture(vec![]).expect("empty index");
        let mut probe = CellProbe { index: &index, lookups: 0 };
        let binding = FixtureBinding { context: edge.snapshot.version.context, static_scope: &edge.scope };
        assert_eq!(
            step_once(&mut edge.carrier, &edge.owner, edge.actor, edge.snapshot,
                &edge.scope, &binding, &mut probe, direction),
            Err(MoveError::CoordinateOverflow)
        );
        assert_eq!(probe.lookups, 0);
        assert_eq!(edge.carrier.read_position(&edge.owner, edge.actor), Ok(edge.snapshot));
    }
}
...
fn recycled_actor_and_stale_owner_reject_before_cell_lookup() {
    ...
    fixture.carrier.remove(&fixture.owner, stale_actor).expect("remove");
    fixture.actor = fixture.carrier.admit(&fixture.owner, ActorState(2)).expect("reuse");
    fixture.snapshot = fixture.carrier.initialize_position(
        &fixture.owner, fixture.actor, stale_snapshot.version.context,
    ...
    let binding = FixtureBinding { context: fixture.snapshot.version.context, static_scope: &fixture.scope };
    assert_eq!(
        prepare_step(&fixture.carrier, &fixture.owner, stale_actor, stale_snapshot,
            &fixture.scope, &binding, &mut probe, Step::East),
        Err(MoveError::Actor(CarrierError::StaleActorGeneration))
    );
    assert_eq!(probe.lookups, 0);
    assert_eq!(fixture.carrier.read_position(&fixture.owner, fixture.actor), Ok(fixture.snapshot));
    fixture.owner.advance(PreProductionContinuityGrant {
    ...
    assert_eq!(probe.lookups, 0);
    assert_eq!(fixture.carrier.slots[0], Slot::Occupied {
...
fn compare_commit_rejects_intervening_write_and_replayed_proposal() {
    ...
    let mut probe = CellProbe { index: &index, lookups: 0 };
    let binding = FixtureBinding { context: fixture.snapshot.version.context, static_scope: &fixture.scope };
    let proposal = prepare_step(&fixture.carrier, &fixture.owner, fixture.actor, fixture.snapshot,
        &fixture.scope, &binding, &mut probe, Step::East).expect("qualified proposal");
    assert_eq!(probe.lookups, 1);
    let intervening = fixture.carrier.compare_commit_position(
        &fixture.owner, fixture.snapshot, fixture.snapshot.version.context,
        LocalPosition { x: 4, y: 6, floor: 7 },
    ).expect("intervening owner commit");
    assert_eq!(commit_step(&mut fixture.carrier, &fixture.owner, proposal),
        Err(MoveError::Actor(CarrierError::PositionSnapshotMismatch)));
    assert_eq!(fixture.carrier.read_position(&fixture.owner, fixture.actor), Ok(intervening));
    let mut replay_probe = CellProbe { index: &index, lookups: 0 };
    assert_eq!(
        prepare_step(&fixture.carrier, &fixture.owner, fixture.actor, fixture.snapshot,
            &fixture.scope, &binding, &mut replay_probe, Step::East),
        Err(MoveError::SnapshotMismatch)
    );
    assert_eq!(replay_probe.lookups, 0);
    assert_eq!(fixture.carrier.read_position(&fixture.owner, fixture.actor), Ok(intervening));

    let fresh_index = fixture.index(LocalPosition { x: 5, y: 6, floor: 7 }, StaticCellFact::Qualified(CollisionClass::Walkable));
    let mut fresh_probe = CellProbe { index: &fresh_index, lookups: 0 };
    let fresh = prepare_step(&fixture.carrier, &fixture.owner, fixture.actor, intervening,
        &fixture.scope, &binding, &mut fresh_probe, Step::East).expect("fresh proposal");
    ...
    assert_eq!(commit_step(&mut fixture.carrier, &fixture.owner, fresh),
        Err(MoveError::Actor(CarrierError::PositionSnapshotMismatch)));
    assert_eq!(fixture.carrier.read_position(&fixture.owner, fixture.actor), Ok(committed));
}
