...
impl std::fmt::Display for StaticCellEngineError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(formatter, "{self:?}")
    }
}
impl std::error::Error for StaticCellEngineError {}
...
impl EngineeringStaticCellIndex {
    pub fn from_claims(
        claims: Vec<EngineeringStaticCellClaim>,
    ) -> Result<Self, StaticCellEngineError> {
        if claims.is_empty() {
            return Err(StaticCellEngineError::Empty);
        }
        ...
        if scope.generation_digest == [0; 32] {
            return Err(StaticCellEngineError::InvalidGeneration);
        }
    ...
    pub fn encode_engineering(&self) -> Result<Vec<u8>, StaticCellEngineError> {
        ...
        write_atom(
            &mut bytes,
            self.scope.content_lock.revision_digest_token.as_str(),
        )?;
        ...
        if bytes.len() > STATIC_CELL_ENGINE_CANDIDATE_MAX_BYTES {
            return Err(StaticCellEngineError::TooManyBytes);
        }
        Ok(bytes)
    }
    ...
    pub fn decode_engineering(bytes: &[u8]) -> Result<Self, StaticCellEngineError> {
        ...
        if cursor.take(8)? != MAGIC {
            return Err(StaticCellEngineError::InvalidEncoding);
        }
        let world_id = WorldId::decode(cursor.take(16)?)
            .map_err(|_| StaticCellEngineError::InvalidEncoding)?;
        let generation_digest: [u8; 32] = cursor
            .take(32)?
            .try_into()
            .map_err(|_| StaticCellEngineError::InvalidEncoding)?;
        let coordinate_frame = CoordinateFrameRef::new(cursor.atom()?)
            .map_err(|_| StaticCellEngineError::InvalidEncoding)?;
        let map_revision = MapRevisionRef::new(cursor.atom()?)
            .map_err(|_| StaticCellEngineError::InvalidEncoding)?;
        let revision_digest_token =
            ProductionAtom::new("engineering Content Lock token", cursor.atom()?)
                .map_err(|_| StaticCellEngineError::InvalidEncoding)?;
        let package_key = ProductionKey::new(cursor.atom()?)
            .map_err(|_| StaticCellEngineError::InvalidEncoding)?;
        let package_revision = ProductionAtom::new("engineering package revision", cursor.atom()?)
            .map_err(|_| StaticCellEngineError::InvalidEncoding)?;
        let digest = std::str::from_utf8(cursor.take(64)?)
            .map_err(|_| StaticCellEngineError::InvalidEncoding)?;
        let package_provenance_digest =
            Sha256HexDigest::new(digest).map_err(|_| StaticCellEngineError::InvalidEncoding)?;
        let count = usize::from(cursor.take(1)?[0]);
        if count == 0 {
            return Err(StaticCellEngineError::Empty);
        }
        if count > STATIC_CELL_ENGINE_CANDIDATE_MAX_CELLS {
            return Err(StaticCellEngineError::TooManyCells);
        }
        ...
        for _ in 0..count {
            ...
            claims.push(EngineeringStaticCellClaim {
                scope: scope.clone(),
        ...
        if index.encode_engineering()?.as_slice() != bytes {
            return Err(StaticCellEngineError::InvalidEncoding);
        }
        Ok(index)
    }
...
fn write_atom(bytes: &mut Vec<u8>, value: &str) -> Result<(), StaticCellEngineError> {
    if value.is_empty() || value.len() > MAX_ATOM {
        return Err(StaticCellEngineError::InvalidEncoding);
    }
    ...
    let next = bytes
        ...
        .ok_or(StaticCellEngineError::Overflow)?;
    if next > STATIC_CELL_ENGINE_CANDIDATE_MAX_BYTES {
        return Err(StaticCellEngineError::TooManyBytes);
    }
...
impl<'a> Cursor<'a> {
    fn take(&mut self, length: usize) -> Result<&'a [u8], StaticCellEngineError> {
        ...
        let result = self
            ...
            .ok_or(StaticCellEngineError::InvalidEncoding)?;
        self.position = end;
        Ok(result)
    }
...
mod tests {
    use super::*;
    fn scope() -> Result<EngineeringStaticCellScope, Box<dyn std::error::Error>> {
        let mut world = [1u8; 16];
        world[6] = 0x71;
        world[8] = 0x81;
        Ok(EngineeringStaticCellScope {
            world_id: WorldId::decode(&world)?,
            coordinate_frame: CoordinateFrameRef::new("engineering-frame")?,
            map_revision: MapRevisionRef::new("engineering-map")?,
            generation_digest: [7; 32],
            content_lock: ContentLockBinding {
                revision_digest_token: ProductionAtom::new("lock", "engineering-lock")?,
                entries: vec![ContentLockEntry::exact(
                    ProductionKey::new("engineering:package")?,
                    ProductionAtom::new("package", "engineering-revision")?,
    ...
    fn claim(
        scope: &EngineeringStaticCellScope,
        x: i32,
        collision: EngineeringCollisionClaim,
    ) -> EngineeringStaticCellClaim {
        EngineeringStaticCellClaim {
            scope: scope.clone(),
            cell: LogicalCell {
                x,
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        for n in [1, 2, 4] {
            ...
            let encoded = index.encode_engineering()?;
            assert!(encoded.len() <= STATIC_CELL_ENGINE_CANDIDATE_MAX_BYTES);
            assert_eq!(
                EngineeringStaticCellIndex::decode_engineering(&encoded),
                Ok(index.clone())
            );
            assert_eq!(
                index.lookup(
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(
            EngineeringStaticCellIndex::from_claims(vec![walk.clone(), walk.clone()]),
            Err(StaticCellEngineError::Duplicate)
        );
        let index = EngineeringStaticCellIndex::from_claims(vec![walk])?;
        ...
        other.generation_digest[0] ^= 1;
        assert_eq!(
            index.lookup(&other, cell),
        ...
        other = scope.clone();
        other.coordinate_frame = CoordinateFrameRef::new("other-frame")?;
        assert_eq!(
            index.lookup(&other, cell),
            Err(StaticCellEngineError::ScopeMismatch)
        );
        other = scope.clone();
        other.map_revision = MapRevisionRef::new("other-map")?;
        assert_eq!(
            index.lookup(&other, cell),
            Err(StaticCellEngineError::ScopeMismatch)
        );
        other = scope.clone();
        other.content_lock.revision_digest_token = ProductionAtom::new("lock", "other-lock")?;
        assert_eq!(
            index.lookup(&other, cell),
            Err(StaticCellEngineError::ScopeMismatch)
        );
        for (value, error) in [
            (
                EngineeringCollisionClaim::Unqualified,
                StaticCellEngineError::Unqualified,
            ),
            (
                EngineeringCollisionClaim::Conflict,
                StaticCellEngineError::Conflict,
            ),
        ] {
            let index = EngineeringStaticCellIndex::from_claims(vec![claim(&scope, 0, value)])?;
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        scope.coordinate_frame = CoordinateFrameRef::new(&atom)?;
        scope.map_revision = MapRevisionRef::new(&atom)?;
        scope.content_lock.revision_digest_token = ProductionAtom::new("lock", &atom)?;
        scope.content_lock.entries[0].package_key =
            ProductionKey::new(&format!("e:{}", "a".repeat(510)))?;
        scope.content_lock.entries[0].package_revision = ProductionAtom::new("revision", &atom)?;
        let mut claims = Vec::new();
        for x in 0..STATIC_CELL_ENGINE_CANDIDATE_MAX_CELLS {
            claims.push(claim(
                &scope,
                i32::try_from(x)?,
                EngineeringCollisionClaim::Qualified(CollisionClass::Blocked),
        ...
        let index = EngineeringStaticCellIndex::from_claims(claims)?;
        let bytes = index.encode_engineering()?;
        assert_eq!(bytes.len(), STATIC_CELL_ENGINE_CANDIDATE_MAX_BYTES);
        assert_eq!(
            EngineeringStaticCellIndex::decode_engineering(&bytes),
            Ok(index)
        );
        assert_eq!(
            EngineeringStaticCellIndex::decode_engineering(&[bytes, vec![0]].concat()),
            Err(StaticCellEngineError::TooManyBytes)
        );
        Ok(())
    }
    #[test]
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        let scope = scope()?;
        let index = EngineeringStaticCellIndex::from_claims(vec![claim(
            &scope,
            0,
            EngineeringCollisionClaim::Qualified(CollisionClass::Blocked),
        )])?;
        let bytes = index.encode_engineering()?;
        assert_eq!(
            EngineeringStaticCellIndex::decode_engineering(&bytes[..bytes.len() - 1]),
            Err(StaticCellEngineError::InvalidEncoding)
        );
        assert_eq!(
            EngineeringStaticCellIndex::decode_engineering(&vec![
                0;
                STATIC_CELL_ENGINE_CANDIDATE_MAX_BYTES
                    + 1
            ]),
            Err(StaticCellEngineError::TooManyBytes)
        );
        ...
        bad[0] ^= 1;
        assert_eq!(
            EngineeringStaticCellIndex::decode_engineering(&bad),
            Err(StaticCellEngineError::InvalidEncoding)
        );
        let mut extra = bytes;
        extra.push(0);
        assert_eq!(
            EngineeringStaticCellIndex::decode_engineering(&extra),
            Err(StaticCellEngineError::InvalidEncoding)
        );
        ...
        invalid.generation_digest = [0; 32];
        assert_eq!(
            EngineeringStaticCellIndex::from_claims(vec![claim(
                &invalid,
                0,
                EngineeringCollisionClaim::Unqualified
            )]),
            Err(StaticCellEngineError::InvalidGeneration)
        );
        invalid.generation_digest = [7; 32];
        ...
        assert_eq!(
            ...
            Err(StaticCellEngineError::InvalidLock)
        );
        Ok(())
    }
}
