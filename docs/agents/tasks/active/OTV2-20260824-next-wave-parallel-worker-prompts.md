...
updated_at: 2026-08-24T17:18:43Z
...
- `DERIVED`: four reusable prep prompts are still missing; #95 already has `OTV2_CONTENT_FORMAT_SPIKE.md`.
- `PROVEN`: prompts do not grant write authority; live Issue/coordinator allocation and GitHub state remain authoritative.

## Acceptance criteria

- [ ] Four preparation prompts exist for #93, #94, #96 and #97.
- [ ] One launch prompt/matrix defines all five preparation lanes and safe parallelism.
- [ ] Existing #95 prompt is reused rather than duplicated.
- [ ] README indexes all new aliases.
- [ ] No runtime, contract, registry, Cargo/workspace, workflow or production authority changes.
...
PR #103 already closed the previously identified execution-clarity gaps. This task only prepares reusable isolated worker contracts for the lawful preparation wave.
...
```yaml
last_progress: issue #106 and branch created from exact main base
status: implementing
branch: docs/next-wave-parallel-workers-20260824
head_sha: null
pr: null
...
blocker: null
next_action: add reusable preparation prompts and launcher
```
