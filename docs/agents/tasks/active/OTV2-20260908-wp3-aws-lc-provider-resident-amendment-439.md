...
- operation-scaled KX start custody; and
...
Conservative process first-use reservation:
...
This covers the AWS-LC TLS pointer table, frontend RAND state, entropy-source object, tree-Jitter thread DRBG and fixed `ERR_STATE`. Registration bookkeeping is separately precharged. Minimum implementation may conservatively retain thread registrations until process teardown; root exhaustion denies before another AWS-LC thread registration. An exact thread-exit release is optional only with proven free-before-release; the pinned x86-64 aws-lc-sys binding exposes `AWSLC_thread_local_clear()`.
...
Initial KX keeps its reservation while active. HRR reserves the complete replacement bound before replacement while the old reservation remains held; old charge releases only after old backing destruction.
...
- [x] Per-thread RNG/error/TLS residency given a finite root-shared reservation.
...
- [x] Exact target operation KX bounds and HRR overlap semantics defined.
...
6. X25519/P-256/P-384/hybrid exact operation max/max+1 denial before key generation;
7. HRR old+replacement overlap and no early old release;
8. hybrid child failure/unwind without leak or early release;
9. completion/error/cancellation/drop release of operation charge only after active backing destruction while process residency persists;
10. ordinary owner-free API unchanged;
11. owner-aware unsupported custom provider fails before ordinary start;
12. exact-target internal size/capacity assertions;
13. actual AWS-LC positive TLS handshake under the budgeted SQLx path;
14. configured PostgreSQL 17.6 qualification and all remaining #356 acceptance cells.
...
The architecture candidate must contain exactly the two allocated documentation paths. Independent review is mandatory because the disposition changes from D to B. Canonical repository checks and normal Merge Queue remain integration authority. No direct protected-main update, bypass, protection weakening or self-approval is authorized.
...
```yaml
last_progress: exact provider/KX finite bounds plus shared-root lifetime model source-closed for qualified target
status: READY_FOR_REVIEW
branch: arch/wp3-aws-lc-provider-resident-boundary-439
pr: pending publication
canonical_material_pr: 356
canonical_material_modified: false
blocker: protected integration of this architecture amendment before material KX lease
next_action: publish exact two-file candidate, verify changed paths, open normal PR, obtain independent review and required CI, integrate through Merge Queue, protected-read back, then apply bounded KX lease to existing #356
```
