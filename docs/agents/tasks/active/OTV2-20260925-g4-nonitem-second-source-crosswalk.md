...
One shared 20,000-page-entry budget and one 20,000-request Fandom network budget (including the five preflight requests) are enforced incrementally across all seven family enumerations. Each listing request sets `eilimit` to no more than the remaining page budget, and any continuation at zero budget stops before another request; per-family 10,000 caps do not add to or bypass the global limit. Revision batches contain only already-budgeted page IDs, and every page/revision request consumes the shared request budget before network access.
