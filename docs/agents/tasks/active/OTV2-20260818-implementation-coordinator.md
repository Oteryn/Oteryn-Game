...
updated_at: 2026-08-22T18:08:00+02:00
...
- `PROVEN`: historical SIM PR #13 bound `worker_base_sha` to the exact allocation merge SHA before implementation; this Wave 1 bind follows the same lifecycle.
...
This bind PR is coordination-only. While it is unmerged, all four worker lanes remain read-only because canonical `main` still contains their `allocated_pending_exact_base` state. After lawful exact-head merge, create every worker branch/task from the resulting post-bind `main`; the live record keeps `worker_base_sha` equal to the exact allocation merge `33cec30b8075c73290d7d76e9f59df4701771650`, matching the established SIM #12 → #13 convention.
...
```yaml
last_progress: Wave 1 allocation PR #45 passed exact-head gates and merged as 33cec30b8075c73290d7d76e9f59df4701771650; a separate coordinator bind branch records that exact allocation merge for all four workers.
status: wave1_exact_base_bind_pending
branch: agent/otv2-coordinator-bind-wave1-base
head_sha: null
pr: pending
blocker: exact-base bind PR must pass exact-head governance/merge checks and merge before any worker write
owner_action_required: null
next_action: open and validate the Wave 1 exact-base bind PR; after lawful merge, create the four worker branches/tasks from resulting main and release their non-overlapping implementation writes.
```
