...
Autonomous protected integration inherits the integration-capability routing selected by the immutable bound META policy; Game prompts and procedures must not narrow that routing to one provider-local primitive. Before declaring an integration-capability blocker, obtain fresh current-session capability evidence and classify it under the bound META router: use `DIRECT_CAPABLE` when the selected exact-head native route is available; otherwise use a freshly proven `DELEGATED_CAPABLE` route through `meta.governed_merge_queue_executor.v1` and its bounded control request. `BLOCKED_CAPABILITY_UNAVAILABLE` is valid only when neither direct nor delegated capability is freshly proven. Both routes preserve exact repository, PR, `base=main`, head, authorization and eligibility fencing plus Merge Queue authority. Direct/immediate merge, generic `enablePullRequestAutoMerge`, bypass, force, a default merge action, no-op/retrigger commits and ambiguous dequeue cleanup remain forbidden substitutes. Queue request or acceptance is not terminal proof; require the real `merge_group` aggregate `game-gate` and protected-main readback before closeout.
