...
The active worker was placed on fail-closed hold in PR #525 comment `5614417475` while its observed head `721a296281bc2085eb4f854241b9e0764abd056a` still contained only the active task record. An already-running Codex material task then raced that hold and later advanced the **same** branch/PR through observed head `f6605bfb482649dbb381b2dfd76f1ba1184242ae` with census helper/tests/evidence. Preserve those commits and branch history, but classify every material result produced after hold `5614417475` and before a future explicit post-repair #162 resume as `PRE_REPAIR_UNQUALIFIED / NOT_ACCEPTED_EVIDENCE`. This quarantine applies to any later pre-resume commit as well as the observed `f6605bf...` head: such counts, byte totals, digests and classifications do not satisfy #511/#504 acceptance and must not be integrated or consumed downstream. After this repair is protected-integrated and read back, #162 must perform a fresh ownership/custody check and explicitly resume the **same** worker branch. The resumed worker must first reconcile that preserved lineage against the repaired protected allocation, update the implementation/evidence to these source/parser/framing rules where required, rerun exact source/parser validation plus census/tests, and regenerate evidence on a post-resume exact head. Only that post-resume qualified evidence may satisfy Phase-A acceptance. Do not create a replacement worker.
