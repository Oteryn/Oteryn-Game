...
6. Allocate Movement after its prerequisites are integration-ready and submit it as the first gameplay integration gate through the current immutable bound META integration-capability router. Use freshly proven `DIRECT_CAPABLE` or `DELEGATED_CAPABLE`; record `BLOCKED_CAPABILITY_UNAVAILABLE` only when neither route is proven and preserve the qualified head. Do not substitute direct merge or generic auto-merge.
