...
6. Allocate Movement after its prerequisites are integration-ready and submit it as the first gameplay integration gate only through the bound META policy’s governed atomic Merge Queue capability. If that capability is unavailable, record `BLOCKED_CAPABILITY_UNAVAILABLE` and preserve the qualified head; do not substitute direct merge or generic auto-merge.
