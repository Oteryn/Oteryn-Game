...
8. When all gates pass, route integration only through the governed atomic Merge Queue capability required by the bound META policy. If that capability is unavailable, record `BLOCKED_CAPABILITY_UNAVAILABLE`, preserve the qualified head and continue safe path-disjoint work; do not substitute direct merge or generic auto-merge.
