...
```yaml
prompt_id: OTV2_SOL_SYSTEMS_ECONOMY_PREP
prompt_version: "1.1"
...
short_invocation: "Oteryn: sol systems economy prep"
```

## Mission

Prepare the future **Systems/Economy** lane from live repository truth after the first Movement+Combat VSL is terminal. Inventory accepted itemization, quests, rewards, crafting, economy and later player-system backlog so a later coordinator/architect can create an exact allocation without forcing Terra to invent technical scope.
...
Do not select item/value/economy formulas, persistence/value custody semantics, monetization/product policy, or mutate system/runtime paths.
