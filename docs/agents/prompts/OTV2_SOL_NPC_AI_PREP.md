...
short_invocation: "Oteryn: sol npc ai prep"
```

## Mission

Prepare the future **NPC/AI** lane from live repository truth after the first Movement+Combat VSL is terminal. Inventory accepted spawn, pathing, perception, NPC interaction, behavior and AI integration backlog so a later coordinator/architect can create an exact allocation without forcing Terra to invent technical scope.
...
Do not invent gameplay/AI semantics, resource maxima, spawn policy, NPC product behavior, or mutate AI/runtime paths.
