...
`OTV2_TERRA_GAME_CONTROL_PLANE.md` remains reusable but is mutually exclusive with Work for mutating control-plane duty. Resolve the unique active control plane from the live coordinator Issue/task; if it is ambiguous, all control-plane mutation fails closed.
