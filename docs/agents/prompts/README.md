...
The former `Oteryn: terra game coordinator` profile is retired and `retired/OTV2_TERRA_GAME_CONTROL_PLANE.md` is provenance only. `OTV2_WORK_DELIVERY_COORDINATOR.md` is the sole reusable mutating Game control-plane profile; live allocation still governs whether it may mutate.
