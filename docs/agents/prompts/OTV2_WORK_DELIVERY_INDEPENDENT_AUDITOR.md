...
- protected integration is routed by the current immutable bound META integration-capability decision after fresh repository/PR/`base=main`/head/auth/eligibility preflight; a freshly proven direct route or freshly proven delegated executor route is acceptable according to the bound contract, and absence of the direct primitive alone is not `BLOCKED_CAPABILITY_UNAVAILABLE`; route-specific receipt/reconciliation evidence is complete; queue request/admission is not terminal proof; real `merge_group` `game-gate` success and protected-main readback confirm the accepted candidate before lifecycle closeout; direct/immediate merge, generic `enablePullRequestAutoMerge`, bypass, force, a default merge action, no-op/retrigger commits and ambiguous automated dequeue are not accepted substitutes; capability is blocked only when neither direct nor delegated route is freshly proven;
