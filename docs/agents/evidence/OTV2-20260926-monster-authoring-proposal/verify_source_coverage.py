"""Account for every inventoried standard registrar/spell path in the formal proposal."""
import json
        ...
        for group in ('mask_paths_with_lines','spell_paths_with_lines'):
            for path,lines in source[group].items():
                ...
                if path in ('mask.raceId','mask.bosstiary.bossRaceId'):
                    ...
                    note='Original script remains blocked until its required behavior is resolved; no Lua execution in this proposal.'
