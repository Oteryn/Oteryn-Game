...
4. if the active connector cannot dispatch or cancel Actions, do not mutate merge state to recover CI and do not arm generic auto-merge. Use `WAITING_EXTERNAL` for an external GitHub/runner dependency. If integration itself is ready, resolve the current immutable bound META integration-capability router first: absence of a direct native operation is not a blocker when a fresh `DELEGATED_CAPABLE` executor route is proven. Record `BLOCKED` with blocker code `BLOCKED_CAPABILITY_UNAVAILABLE` only when neither direct nor delegated integration capability is proven, preserve the qualified candidate and continue any safe path-disjoint work;
