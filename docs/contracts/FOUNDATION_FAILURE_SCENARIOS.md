...
| `FS-KEY-ROTATION` | Signing/verification key rotates or is emergency-revoked during login/recovery | Valid overlap and revocation behavior; no acceptance outside current trusted policy, and a recovery PREPARE cannot escrow a signing key/profile that is revoked before COMMIT. |
...
| `FS-RECONNECT-PREPARE-COMMIT-ELIGIBILITY-CHANGE` | Rebind PREPARE succeeds, then incumbent health, grant time/nonce/key-profile trust, Platform-security validity, signed recovery compatibility, lease/runtime/session authority, fencing/handoff/terminality or reconciliation eligibility changes before COMMIT | COMMIT revalidates current authority/security/recovery-key-profile trust/compatibility atomically. A stale prepared candidate cannot revive or overwrite whatever authority state is current at revalidation, cannot advance its candidate generation, cannot consume a recovery grant as success and cannot create partial authority. Recovery key/profile revocation after PREPARE MUST reject as `RECOVERY_GRANT_AUTHENTICATION_FAILED` before authority mutation or RecoveryGrantNonce consumption and MUST preserve whatever authority state is current at revalidation; if signed recovery `compatibility_revision` became unsupported/superseded, reject as `RECOVERY_GRANT_REVISION_UNSUPPORTED` with no RecoveryGrantNonce consumption or authority mutation. |
