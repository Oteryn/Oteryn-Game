...
| `FS-RECONNECT-PREPARE-COMMIT-ELIGIBILITY-CHANGE` | Rebind PREPARE succeeds, then incumbent health, grant/security validity, signed recovery compatibility, lease/runtime/session authority, fencing/handoff/terminality or reconciliation eligibility changes before COMMIT | COMMIT revalidates current authority/security/compatibility atomically. A stale prepared candidate cannot revive or overwrite whatever authority state is current at revalidation, cannot advance its candidate generation, cannot consume a recovery grant as success and cannot create partial authority. If signed recovery `compatibility_revision` became unsupported/superseded, reject as `RECOVERY_GRANT_REVISION_UNSUPPORTED` with no RecoveryGrantNonce consumption or authority mutation. |
