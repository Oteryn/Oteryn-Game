...
| `FS-RECOVERY-GRANT-ISSUANCE-AMBIGUITY` | Recovery-grant issuer loses the response or crashes after a grant may already have been issued | Return the contract-owned reconciliation result; retry only the same producer attempt, mint no blind second recovery grant, and require deterministic retirement plus proof that any possibly issued capability is no longer acceptable before an independent new recovery attempt. Producer ambiguity creates no gameplay or fresh-entry authority. |
...
| `FS-KEY-ROTATION` | Signing/verification key rotates, trust evidence becomes stale/unavailable, or a key/profile is emergency-revoked during login/recovery | Valid overlap and revocation behavior; no acceptance outside current trusted policy. FND-04 grant profiles require authenticated signing-key/profile trust/revocation evidence within their accepted freshness ceiling at the authority-changing boundary. Stale/unavailable/unprovable trust evidence fails closed under the purpose-specific stale-security-evidence progression, while fresh explicit revocation/unknown trust fails under the purpose-specific authentication-failed progression; neither consumes the grant nonce or creates authority. A PREPARE or earlier validation cannot escrow trust. |
...
| `FS-RECONNECT-PREPARE-COMMIT-ELIGIBILITY-CHANGE` | Rebind PREPARE succeeds, then incumbent health, grant time/nonce/key-profile trust or trust-evidence freshness, Platform-security validity, signed recovery compatibility, lease/runtime/session authority, fencing/handoff/terminality or reconciliation eligibility changes before COMMIT | COMMIT revalidates current authority/security/recovery-key-profile trust **and trust-evidence freshness**/compatibility atomically. A stale prepared candidate cannot revive or overwrite whatever authority state is current at revalidation, cannot advance its candidate generation, cannot consume a recovery grant as success and cannot create partial authority. Recovery key/profile trust/revocation evidence that is stale, unavailable, unauthenticated, contradictory or cannot prove current trust MUST reject as `RECOVERY_GRANT_SECURITY_EVIDENCE_STALE` before authority mutation or RecoveryGrantNonce consumption; fresh current evidence explicitly showing key/profile revocation MUST reject as `RECOVERY_GRANT_AUTHENTICATION_FAILED`; if signed recovery `compatibility_revision` became unsupported/superseded, reject as `RECOVERY_GRANT_REVISION_UNSUPPORTED`. Every failure preserves whatever authority state is current at revalidation. |
