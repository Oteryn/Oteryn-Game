...
**Operator OS identity.** The tool never runs as the node's service user. It runs as a separate privileged operator identity (root, or an account holding only the file-ownership capability), whose control-plane credential files are owned by that identity with mode 0600 and are never readable by the service UID. A running or compromised node therefore cannot read the control-plane credential. Every file the node must read (the launch authorization and the Character recovery-fence files) is created by the tool with mode 0600, then its ownership is transferred to the service user through the open descriptor before content is written, then it is synchronized. The node's owner check then accepts it, and no path-based ownership change is ever made. The tool refuses to run as the service user.
...
- **Operator identity.** `oteryn-game-ops` refuses to run as the service user. Its control-plane credential file is unreadable by the service UID. A launch authorization and fence files it creates are accepted by the node's owner checks.
