...
- **Contents:**
  ...
  - two distinct authenticated namespaces over that one channel, each distinct from the TLS peer name:
    ...
    - the Character intent `issuer_authority`, which is not configured: it is the compiled constant `OTERYN_PLATFORM_CHARACTER_AUTHORITY` (`character_bootstrap_intent::ISSUER_AUTHORITY`) that every intent must name (#414).
...
**Operator OS identity.** The tool never runs as the node's service user. It runs as root, the one supported operator identity in this slice, whose control-plane credential files are owned by that identity with mode 0600 and are never readable by the service UID. A running or compromised node therefore cannot read the control-plane credential. Every file the tool writes that the node must read (the launch authorization, the S2 fresh-store authorization and the Character recovery-fence files; the rule applies to any such file, not only this list) is created by the tool with mode 0600, then its ownership is transferred to the service user through the open descriptor before content is written, then it is synchronized. The node's owner check then accepts it, and no path-based ownership change is ever made. The tool refuses to run as the service user.

**Durable request and authorization files.** They are created only beneath one configured operator state directory. Before creating any of them, the tool validates that directory and every ancestor up to `/`: each is owned by root or the invoking user, none is writable by group or others (except a sticky-bit root-owned ancestor, where only its owner can rename entries), and each is opened relative to its validated parent without following symbolic links. Otherwise it refuses. Creation then happens relative to the validated directory descriptor, so no path component can be renamed away from under the retained request. Every file the tool writes (launch authorization, assignment request, Character recovery request) is created as a new file (exclusive create, no symbolic-link following, mode 0600), written completely, synchronized, and then its parent directory is synchronized. Only after all of this succeeds does the tool submit the database or fence mutation that the file guards. Any failure before that point aborts without the mutation, so a crash can never leave a committed or ambiguous mutation whose exact request was lost. It never registers a GameNode incarnation, so `NodeId` keeps its ADR-0009 meaning: the identity of a running game-server process. Its actions, all through existing Game-owned control-plane operations:
...
- **The S2 fresh-store authorization.** It is required by the S2 evidence decision: a genuinely new store initializes only under an independently authorized fresh-store provenance record. The tool issues it as a file holding the complete `FreshStoreProvenance`: namespace, authorization reference, source authority and the exact `initialized_at` timestamp fixed at issuance. The file also holds the descriptor registration revision, `installed_at` and facts. Because every field is fixed at issuance, an ambiguous initialization can be compared exactly on restart. Issuing it never initializes the store itself.
  - **Durable, control-plane-authenticated issuance.** The file alone proves nothing. The tool first writes it durably, then records the exact canonical content through a new control-plane operation into a new issuance table keyed by descriptor revision. The one issuance covers the whole Platform producer (the Character `issuer_authority` is the compiled constant and is not an issued value), so it is recorded once, before the node starts. Only the control-plane role may insert, and the recorded actor comes from the authenticated session role. An exact replay returns success; different content rejects.
...
- The operator supplies only the intent's operation id, over a node-local Unix-domain control socket. The socket is created mode 0600, owned by the node's service user and never network-reachable. Root reaches it despite the 0700 directory and 0600 mode. On every accepted connection the node reads the peer credentials (`SO_PEERCRED`) and serves only uid 0, the operator identity (D2). Any other peer, including the service user itself, is closed without reading a request.
