...
- destination `AccountId` does not already consume its one ordinary-house allowance for that `WorldId`;
...
If those conditions are not met, the Character+house Bazaar settlement must fail or remain in an explicit non-committed reconciliation state. It must not silently discard the house, bypass the account cap, or create a second authoritative owner.
...
`YES` for the first-generation exchange surface and Character Bazaar disposition.

Without this decision, later Bazaar and housing contracts could accidentally create either an unintended private property market or an unconditional rule that blocks Character sale whenever a house exists.
...
5. guildhouses do not follow the Character sale;
6. World transfer still cannot move the physical address.
...
`BUYER HOUSING CAP / ELIGIBILITY: MUST REVALIDATE BEFORE SETTLEMENT`
