...
Use separate closed typed response families, not a nullable mega-event. JSON decoding rejects duplicate or unknown members, wrong types, noncanonical values and unsupported versions. Version is integer `1`; comparable revision/generation values are positive unsigned 64-bit decimal strings without signs/leading zeroes; Unix seconds are nonnegative decimal strings fitting signed 64 bits. `clock_uncertainty_seconds` is a canonical unsigned 64-bit decimal JSON string, allowing `"0"`; signs, leading zeroes other than the single-digit zero representation, fractions and values outside the unsigned 64-bit range reject. Its subsequent checked conversion/arithmetic and conservative source-age validation remain mandatory: a representable wire value that overflows those checks or cannot prove the accepted freshness bound still rejects. UUIDs use lowercase hyphenated canonical full UUID strings and their existing semantic validation. Public keys use unpadded base64url of exactly 32 Ed25519 bytes. Existing fixed issuer/profile/purpose constants remain unchanged.
