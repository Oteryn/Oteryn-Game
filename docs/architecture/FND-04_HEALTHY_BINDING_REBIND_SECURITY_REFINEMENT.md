...
| 30s max grant lifetime, 5s verifier skew, <=5s Platform-security evidence age | `YES` for v1 ceilings | producer/consumer acceptance and revocation behavior | loosening/tightening deployed windows later changes replay/revocation exposure and can break clients/operators relying on old timing behavior | measured timing/distribution evidence + threat-model review | actual shorter producer lifetime, refresh cadence, cache implementation and operational SLOs inside these ceilings |
