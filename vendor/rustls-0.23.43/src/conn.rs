...
impl<Data> ConnectionCommon<Data> {
    /// Installs accounting for the private incoming TLS deframer backing.
    ///
    /// Installation reserves any existing capacity before retaining `owner`.
    /// Subsequent read growth is denied before allocation and reader invocation.
    #[cfg(feature = "std")]
    pub fn set_deframer_buffer_owner(
        &mut self,
        owner: crate::sync::Arc<dyn DeframerBufferOwner>,
    ) -> Result<(), DeframerBufferError> {
        self.deframer_buffer.set_owner(owner)
    }
