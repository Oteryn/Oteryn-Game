...
impl PgConnectOptions {
    pub(crate) fn parse_from_url(url: &Url) -> Result<Self, Error> {
        let options = Self::parse_url_into(Self::new_without_pgpass(), url)?;
        Ok(options.apply_pgpass())
    }

    /// Parse a PostgreSQL URL without consulting environment variables, passfiles,
    /// or operating-system identity defaults.
    pub fn from_str_without_environment(s: &str) -> Result<Self, Error> {
        let url: Url = s.parse().map_err(Error::config)?;
        Self::parse_url_into(Self::new_without_environment(), &url)
    }

    fn parse_url_into(mut options: Self, url: &Url) -> Result<Self, Error> {
        ...
        for (key, value) in url.query_pairs().into_iter() {
            match &*key {
                ...
                "hostaddr" => {
                    let host_addr = value.parse::<IpAddr>().map_err(Error::config)?;
                    options = options.host_addr(host_addr)
                }
    ...
    pub(crate) fn build_url(&self) -> Url {
        ...
        if let Some(host_addr) = &self.host_addr {
            url.query_pairs_mut().append_pair("hostaddr", host_addr);
        }
...
#[test]
fn it_parses_hostaddr_without_overwriting_tls_host() {
    let url = "postgres:///?host=google.database.com&hostaddr=8.8.8.8";
    let opts = PgConnectOptions::from_str_without_environment(url).unwrap();
