    ...
    for index, entry in enumerate(entries):
        ...
        if status == "reusable":
            if reusable is not True:
                errors.append(f"prompt lifecycle entry {index} has inconsistent reusable status/flag")
...
def _markdown_line_records(text: str) -> list[tuple[int, int, str, bool]]:
    ...
    for segment in text.splitlines(keepends=True):
        ...
        if fence_char is not None:
            inert = True
            close = re.fullmatch(rf" {{0,3}}{re.escape(fence_char)}{{{fence_len},}}[ \t]*", raw)
...
def _remove_default_ignorables(value: str) -> str:
    return "".join(character for character in value if not any(start <= ord(character) <= end for start, end in DEFAULT_IGNORABLE_RANGES))
...
def _has_protected_action(text: str) -> bool:
    return bool(PROTECTED_HOST_ACTION_RE.search(text) or DIRECT_CONNECTOR_RE.search(text) or CONNECTOR_ACTION_RE.search(text))
...
EXEMPTION_RELATION_RE = re.compile(
    r"\b(?:without|absent)\b|\b(?:requires?|needs?)\s+no\b|\b(?:does|do)\s+not\s+(?:require|need)\b"
    r"|\b(?:exempt(?:ed|ion)?\s+from|free\s+(?:from|of))\b|\bno\b.{0,56}\b(?:is|are|was|were)?\s*(?:required|needed|necessary)\b"
    r"|\b(?:not\s+required|not\s+needed|unnecessary|optional|waived|bypassed)\b|\b(?:waive[sd]?|waiving|bypass(?:es|ed|ing)?)\b|\bwith\s+no\b",
    re.IGNORECASE,
)
...
def _any_spans_related(left: list[tuple[int, int]], right: list[tuple[int, int]], max_gap: int) -> bool:
    return any(_span_gap(a, b) <= max_gap for a in left for b in right)


def _authority_is_explicitly_denied(text: str) -> bool:
    if re.search(r"\bno\b.{0,56}\b(?:authority|authorization|authorisation|approval|permission|exception|entitlement)\b", text, re.IGNORECASE):
        return True
    if re.search(r"\b(?:not|never)\b.{0,36}\b(?:authorized|authorised|approved|permitted|allowed|granted|given|retained|automatic(?:ally)?|standing|default)\b", text, re.IGNORECASE):
        return True
    return bool(re.search(r"\b(?:authority|authorization|authorisation|approval|permission|exception|entitlement)\b.{0,48}\b(?:is|are|was|were|be|been|being|remains?)\s+not\b", text, re.IGNORECASE))
...
def _is_blanket_authority_claim(text: str) -> bool:
    ...
    for subject in subjects:
        for authority in authority_spans:
            if _span_gap(subject, authority) > 72:
                continue
            window = _relation_window(text, subject, authority, padding=20)
            if not _authority_is_explicitly_denied(window):
                return True
    for subject in subjects:
        window = _relation_window(text, subject, padding=72)
        if DEFAULT_AUTHORITY_RELATION_RE.search(window) and not _authority_is_explicitly_denied(window):
            return True
    return False
...
def _enforcement_match_is_negated(text: str, start: int, end: int) -> bool:
    ...
    boundaries = list(CONTRASTIVE_BOUNDARY_RE.finditer(prefix))
    if boundaries:
        prefix = prefix[boundaries[-1].end():]
    suffix = text[end:min(len(text), end + 48)]
    if re.search(
        r"\b(?:no|not|never)\b(?:\s+[A-Za-z_-]+){0,4}\s*$|\b(?:doesn't|don't|isn't|aren't|wasn't|weren't|won't|can't|cannot)\b(?:\s+[A-Za-z_-]+){0,4}\s*$",
        prefix,
        re.IGNORECASE,
    ):
        return True
    return bool(re.match(r"\s+(?:is|are|was|were|will|would|can|could|must|may|should)\s+not\b", suffix, re.IGNORECASE))
...
ENFORCEMENT_FAILURE_RELATION_RE = re.compile(
    r"\b(?:calls?|requests?|invocations?|actions?|operations?)\b.{0,64}\b(?:lacking|missing|without|unauthori[sz]ed|unapproved)\b.{0,56}\b(?:per[- ]?action|authorization|authorisation|approval|permission|decision|gate)\b"
    r"|\b(?:per[- ]?action|authorization|authorisation|approval|permission|decision|gate)\b.{0,56}\b(?:missing|lacking|failure|failures|failed|denied|unauthori[sz]ed)\b.{0,64}\b(?:calls?|requests?|invocations?|actions?|operations?)\b",
    re.IGNORECASE,
)
NOMINAL_GATE_ENFORCEMENT_RE = re.compile(
    r"\b(?:routing\s+)?enforcement\b.{0,56}\b(?:per[- ]?action|authorization|authorisation|approval|permission|decision|gate|routing)\b",
    re.IGNORECASE,
)


def _provider_enforcement_has_target(text: str, enforcement: re.Match[str]) -> bool:
    start = max(0, enforcement.start() - 112)
    end = min(len(text), enforcement.end() + 176)
    window = text[start:end]
    return bool(
        DIRECT_GATE_OBJECT_RE.search(window)
        or ENFORCEMENT_FAILURE_RELATION_RE.search(window)
        or NOMINAL_GATE_ENFORCEMENT_RE.search(window)
    )


def _is_provider_enforcement_claim(text: str) -> bool:
    providers = list(PROVIDER_NOUN_RE.finditer(text))
    enforcements = [match for match in ENFORCEMENT_CONCEPT_RE.finditer(text) if not _enforcement_match_is_negated(text, match.start(), match.end())]
...
def _outside_routing_paragraphs(text: str) -> list[str]:
    ...
    for paragraph in paragraphs:
        ...
        if len(lines) > 1 and all(line.startswith("- ") for line in lines):
            units.extend(line for line in lines if _is_outside_routing_policy(_normalize_policy_text(line)))
            continue
...
def _validate_outside_routing_text(path: str, outside_text: str, approved: set[str], errors: list[str]) -> None:
    for paragraph in _outside_routing_paragraphs(outside_text):
        if paragraph not in approved:
            snippet = paragraph.replace("\n", " ")[:160]
            errors.append(f"{path}: Remote Desktop policy text outside canonical section: {snippet!r}")
...
def _validate_routing_adjacent_sections(path: str, text: str, errors: list[str]) -> None:
    ...
    for heading, expected in expected_sections.items():
        ...
        if len(matches) != 1 or matches[0] != expected:
            errors.append(f"{path}: routing list section {heading!r} must match exactly")
...
def _normalize_github_host(value: str) -> str:
    ...
    aliases = {"www.github.com": "github.com", "raw.github.com": "raw.githubusercontent.com", "www.raw.githubusercontent.com": "raw.githubusercontent.com"}
...
def _github_routing_reference(candidate: str) -> tuple[str, str] | None:
    ...
    if host == "api.github.com":
        ...
        if lowered == expected:
            ...
            refs = [_decode_url_component(value) for key, values in query.items() if _decode_url_component(key).lower() == "ref" for value in values]
...
def _validate_meta_routing_coordinates(path: Path, text: str, errors: list[str]) -> None:
    ...
    for coordinate in META_ROUTING_COORDINATE_RE.findall(operative):
        if coordinate != EXPECTED_META_ROUTING_COORDINATE:
            errors.append(f"{path}: stale META execution-routing coordinate {coordinate!r}; expected {EXPECTED_META_ROUTING_COORDINATE!r}")
    for candidate in _github_url_candidates(text):
        ...
        if selector != META_SHA:
            errors.append(f"{path}: stale META execution-routing coordinate in {label} {candidate!r}; expected selector {META_SHA!r}")
...
def validate() -> list[str]:
    ...
    if not errors:
        print(f"Validated Remote Desktop per-action routing for {len(prompt_paths)} reusable prompts and {len(CANONICAL_SURFACES)} canonical surfaces against META {META_SHA}.")
    return errors
