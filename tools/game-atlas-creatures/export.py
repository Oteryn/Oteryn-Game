...
def parse_spawns(world_root: Path, kind: str):
    records = []; expected = "monsters" if kind == "monster" else "npcs"
    paths = sorted(world_root.rglob(f"*-{kind}.xml"), key=lambda p: p.relative_to(world_root).as_posix())
    for path in paths:
        root = ET.parse(path).getroot()
        if root.tag != expected: raise ExportError(f"{path}: expected <{expected}>")
        for group_index, group in enumerate(root):
            if group.tag != kind: raise ExportError(f"{path}: unexpected <{group.tag}>")
            required_group = {"centerx","centery","centerz","radius"}
            if not required_group <= set(group.attrib): raise ExportError(f"{path}: incomplete {kind} group")
            cx,cy,cz,radius = (int(group.attrib[k]) for k in ("centerx","centery","centerz","radius"))
            for ordinal, entry in enumerate(group):
                if entry.tag != kind or not {"name","x","y","z","spawntime"} <= set(entry.attrib): raise ExportError(f"{path}: malformed {kind} spawn")
                x,y,z = cx+int(entry.attrib["x"]), cy+int(entry.attrib["y"]), int(entry.attrib["z"]); name=entry.attrib["name"]
                records.append({"kind":kind,"name":name,"position":{"x":x,"y":y,"floor":-z},"spawn_area":{"center":{"x":cx,"y":cy,"floor":-cz},"radius":radius},"spawn_time_seconds":int(entry.attrib["spawntime"]),"direction":int(entry.attrib["direction"]) if "direction" in entry.attrib else None,"weight":int(entry.attrib["weight"]) if "weight" in entry.attrib else None,"origin":_origin(path,world_root),"record_id":stable_id(kind,path.relative_to(world_root).as_posix(),group_index,ordinal,name,x,y,z)})
    return records
...
def main() -> int:
    import argparse
    parser=argparse.ArgumentParser(description=__doc__); parser.add_argument("world_root",type=Path); parser.add_argument("npc_root",type=Path); parser.add_argument("monster_root",type=Path); parser.add_argument("output",type=Path)
    args=parser.parse_args(); result=export_creatures(args.world_root,args.npc_root,args.monster_root); args.output.parent.mkdir(parents=True,exist_ok=True); args.output.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n",encoding="utf-8"); print(json.dumps({"semantic_digest":result["semantic_digest"],**result["statistics"]},sort_keys=True)); return 0
