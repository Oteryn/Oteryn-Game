...
def run():
    ...
    assert set(page) == g4.PAGE_FIELDS
    assert page["source_namespace"] == "mediawiki/tibiawiki.com.br"
    assert page["external_id"] == "7"
    assert page["page_key"] == "mediawiki/tibiawiki.com.br/page_id/7"
    ...
    broken_mapping = dict(page)
    broken_mapping["external_id"] = "007"
    expect_error("CAPTURE_EXTERNAL_ID_MAPPING_INVALID", lambda: g4.validate_capture_rows([broken_mapping]))
    broken_key = dict(page)
    broken_key["page_key"] = "mediawiki/tibiawiki.com.br/page_id/8"
    expect_error("CAPTURE_PAGE_KEY_MAPPING_INVALID", lambda: g4.validate_capture_rows([broken_key]))
    expect_error("CAPTURE_IDENTITY_DUPLICATE", lambda: g4.validate_capture_rows([page, page.copy()]))
    missing_field = dict(page)
    del missing_field["source_namespace"]
    expect_error("CAPTURE_ROW_SCHEMA_INVALID", lambda: g4.validate_capture_rows([missing_field]))
