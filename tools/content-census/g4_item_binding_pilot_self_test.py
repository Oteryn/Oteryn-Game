...
def main() -> None:
    ...
    candidate = MODULE.typed_binding_candidate("oteryn:item.example", "capture-batch", "12345")
    assert set(candidate) == {"source_key", "source_revision", "identity_namespace", "external_id", "target", "disposition"}
    assert candidate == {
        "source_key": "oteryn:source.tibiawiki",
        "source_revision": "capture-batch",
        "identity_namespace": "mediawiki/page_id",
        "external_id": "12345",
        "target": {"family": "Item", "key": "oteryn:item.example", "revision": "definition-r1"},
        "disposition": "EXACT",
    }
    try:
    ...
    protected = {"full_output": {"stable_without_retrieval_timestamp_sha256": MODULE.EXPECTED_CURRENT_STABLE_SHA256}}
    manifest = {"full_output": {"stable_without_retrieval_timestamp_sha256": MODULE.EXPECTED_CURRENT_STABLE_SHA256}}
    assert MODULE.verify_stable_current_digest(MODULE.EXPECTED_CURRENT_STABLE_SHA256, manifest, protected) is True
    drifted = {"retrieval_timestamp": "later", "records": [{"source_item_id": 1, "native_key": "oteryn:item.example", "page_id": 54321}], "pages": []}
    drift_digest = MODULE.stable_current_digest(drifted)
    drift_manifest = {"full_output": {"stable_without_retrieval_timestamp_sha256": drift_digest}}
    assert MODULE.verify_stable_current_digest(drift_digest, drift_manifest, protected) is False
    assert protected["full_output"]["stable_without_retrieval_timestamp_sha256"] == MODULE.EXPECTED_CURRENT_STABLE_SHA256
