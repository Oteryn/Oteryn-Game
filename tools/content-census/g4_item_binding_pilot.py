...
def verify_capture(capture: dict[str, Any], manifest: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for row in rows:
        ...
        if row.get("source") != SOURCE_KEY or row.get("source_namespace") != "mediawiki/tibiawiki.com.br":
...
def verify_current(current: dict[str, Any], manifest: dict[str, Any], protected: dict[str, Any]) -> list[dict[str, Any]]:
...
def compile_pilot(capture: dict[str, Any], capture_manifest: dict[str, Any], current: dict[str, Any], current_manifest: dict[str, Any], protected_current_manifest: dict[str, Any], crosswalk: dict[str, Any], crosswalk_raw: bytes) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    matched_rows = verify_current(current, current_manifest, protected_current_manifest)
    ...
    for candidate in matched_rows:
        ...
        if page is None or capture_row is None:
            preliminary.append({"target_key": target_key, "source_item_id": source_id, "external_id": external_id, "status": "UNRESOLVED_SOURCE_SHAPE", "reason": "PAGE_NOT_PRESENT_IN_BOTH_PINNED_INPUTS", "matched_signals": [], "contradicted_signals": []})
            continue
        if (page.get("revision_id") != capture_row.get("revision_id")
            ...
            preliminary.append({"target_key": target_key, "source_item_id": source_id, "external_id": external_id, "status": "UNRESOLVED_SOURCE_SHAPE", "reason": "EXACT_CAPTURE_REVISION_OR_DIGEST_DRIFT", "matched_signals": [], "contradicted_signals": []})
            continue
        if page.get("infobox_present") is not True:
            preliminary.append({"target_key": target_key, "source_item_id": source_id, "external_id": external_id, "status": "UNRESOLVED_SOURCE_SHAPE", "reason": "NO_ITEM_INFOBOX_AT_PINNED_REVISION", "matched_signals": [], "contradicted_signals": []})
            continue
        ...
        preliminary.append({
            ...
            "source_revision_id": capture_row["revision_id"],
            "source_digest": capture_row["source_digest"],
        })
    ...
    evidence = {
        ...
        "typed_binding_candidates": [
            {
                "target": {"family": "Item", "key": row["target_key"], "revision": "definition-r1"},
                "source": {"key": SOURCE_KEY, "revision": source_batch_digest},
                "identity_namespace": IDENTITY_NAMESPACE,
                "external_id": row["external_id"],
                "binding_disposition": "EXACT",
                "candidate_only": True,
            }
            for row in selected
        ],
