...
def transform(census: dict[str, Any], source_manifest: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    allowed_shapes = {"INFOBOX_ITEM", "INFOBOX_ITEM_PARSE_ERROR", "NO_INFOBOX_ITEM"}
    for page in pages:
        ...
        shape = page.get("source_shape")
        if not isinstance(shape, str) or shape not in allowed_shapes:
            raise CaptureError("PAGE_SOURCE_SHAPE_INVALID")
        actual_shapes[shape] += 1
    ...
    expected_shapes = {name: int(source_counts.get(name, 0)) for name in sorted(allowed_shapes)}
    if actual_shapes != Counter({name: count for name, count in expected_shapes.items() if count}):
        raise CaptureError("CENSUS_SOURCE_SHAPE_PARTITION_MISMATCH")
    source_invariants = source_manifest.get("invariants")
    census_shapes = census.get("counts", {}).get("source_shapes")
    if (not isinstance(source_invariants, dict)
            or source_invariants.get("source_shape_partition_complete") is not True
            or census_shapes != source_counts
            or len(rows) != expected
            or sum(expected_shapes.values()) != expected):
