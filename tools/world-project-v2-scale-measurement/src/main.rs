...
fn edit_amplification(total: usize) -> Result<serde_json::Value, Box<dyn Error>> {
    ...
    Ok(json!({
        "definition": "all canonical project bytes rewritten by one-placement edit / changed bytes in that placement record",
        "changed_placement_record_bytes": changed,
        "full_project_rewrite_bytes": total,
        "amplification_ratio": total as f64 / changed as f64
    }))
}
